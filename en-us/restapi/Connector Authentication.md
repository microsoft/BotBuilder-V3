...
2. OAuth 2.0 login to the Microsoft Account (MSA) login server is used to generate a secure token that a bot can use to send messages. This token is a service-to-service token; no user login is required.
...
### (Bot -> Connector) Step 1: POST to the MSA login service to retrieve a token
...
Example request to the MSA server:
...
### (Bot -> Connector) Step 2: Receive JSON containing JWT token from MSA login service

If your application is authorized by the MSA service, it will return a JSON payload containing your access token, its type, and its expiration (in seconds). The should is used in its existing form, without further escaping or encoding. It may be used until its expiration, and we encourage bot developers to cache and proactively refresh this token to keep it from impacting your bot's performance.

Example response from the MSA server:
...
**NOTE**: DO NOT include this header on HTTP requests to other services. The JWT token from the MSA service is like a password -- it allows the holder of this token to perform operations on behalf of your bot. **Do not send your JWT token over unsecured channels, and do not send it to services other than the Bot Connector**.
