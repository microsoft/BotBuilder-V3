...
namespace Microsoft.Bot.Builder.FormTest
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var callDebug = new CallDialog("Root", debugForm, (session, result) =>
            {
                var testResult = result as FormResult<Choices>;
                if (testResult != null)
                {
                    switch (testResult.State.Choice)
                    {
                        case DebugOptions.AnnotationsAndNumbers: return session.BeginDialogAsync(annotationsAndNumbers);
                        case DebugOptions.AnnotationsAndNoNumbers: return session.BeginDialogAsync(annotationsAndWords);
                    }
                }
                return session.EndDialogAsync(dialogs.Get("Root"), result);
            });
