...
namespace Microsoft.Bot.Builder.Form
{
         ...
         where T : class, new()
    {
        ...
        public virtual Task<DialogResponse> BeginAsync(ISession session, object args = null)
        {
            try
            {
                var initialState = args as InitialState;
                ...
                session.Stack.SetDialogState<FormState>(_id, form);
                return ReplyReceivedAsync(session);
            }
            catch (Exception exception)
            {
                return session.EndDialogAsync(this, new FormResult<T>(_id, exception));
            }
        }
        ...
        */
        ...
        public virtual async Task<DialogResponse> ReplyReceivedAsync(ISession session)
        {
            try
            {
                var form = session.Stack.GetDialogState(_id) as FormState;
                var state = session.SessionData.PerUserInConversationData[_id] as T;
                string message = null;
                string prompt = null;
                ...
                {
                    FormResult<T> result = new FormResult<T>(_id);
                    if (next.Direction == StepDirection.Complete)
                    {
                        ...
                        result = new FormResult<T>(_id, state);
            ...
            {
                return session.CreateDialogErrorResponse(System.Net.HttpStatusCode.InternalServerError, exception.ToString()).Result;
            }
        }

        public virtual Task<DialogResponse> DialogResumedAsync(ISession session, DialogResult result)
        {
            var form = session.Stack.GetDialogState(_id) as FormState;
            ++form.Step;
            form.StepState = null;
        ...
        protected class FieldStep : IStep
        {
            ...
            public string Help(T state, FormState form, string commandHelp)
            {
                ...
                return "* " + template.Prompt(state, _name, "* " + template.Recognizer().Help(_field.GetValue(state)), commandHelp);
            }
            ...
            protected IPrompt<T> Template(TemplateUsage usage, IRecognizer recognizer = null)
            {
            ...
            protected IPrompt<T> NextClarifyPrompt(T state, FieldStepState stepState, IRecognizer recognizer, out Ambiguous clarify)
            {
                ...
                if (clarify != null)
                {
                    ...
                    var helpTemplate = _field.Template(template.Annotation().AllowNumbers != BoolDefault.No ? TemplateUsage.HelpOneNumber : TemplateUsage.HelpManyNumber).ApplyDefaults(_form.Configuration().DefaultPrompt);
                    var choiceRecognizer = new EnumeratedRecognizer("", null,
                        clarify.Values,
        ...
        protected class NavigationStep : IStep
        {
            public NavigationStep(string name, Form<T> form, T state, FormState formState)
            {
                ...
                var recognizer = new EnumeratedRecognizer(Name(), null, formState.Next.Names,
        ...
        protected void BuildCommandRecognizer()
        {
            ...
            _commands = new EnumeratedRecognizer("Form commands", null,
                values,
        ...
        protected IRecognizer _commands;
