...
namespace Microsoft.Bot.Sample.SimpleAlarmBot
{
    ...
    public class SimpleAlarmBot : LuisDialog<string, Task<object>>
    {
        ...
        /// <returns></returns>
        [LuisIntent("")]
        public Task<Connector.Message> None(ISession session, LuisResult result)
        {
        ...
        public async Task<Connector.Message> AlarmOperation(ISession session, LuisResult result)
        {
            ...
            if (sendEntities)
            {
                reply.Text += string.Format(" entities: {0}", entities);
            }
        ...
        public async Task<Connector.Message> AlarmDialogResumeHandler(ISession session, Task<bool> taskResult)
        {
            ...
            return await session.CreateDialogResponse("Ok! We haven't modified your alarms!");
        }
    }
}