...
declare module 'skype-botkit' {
    export function messagingHandler(botService: BotService): (req, res) => void;
    function ensureHttps(redirect, errorStatus): (req, res, next) => void;
    function verifySkypeCert(options): (req, res, next) => void; 
    ...
    export class Bot {
        reply(content: string, callback: ISendMessageCallback);
        send(to: string, content: string, callback: ISendMessageCallback);
    }
}